...
module Hpricot
  class XMLDecl
    def initialize(version, encoding=nil, standalone=nil)
      ...
      if /\A[a-zA-Z0-9_.:-]+\z/ !~ version
        raise Hpricot::Error, "invalid version in XML declaration: #{version.inspect}"
      end
      if encoding && /\A[A-Za-z][A-Za-z0-9._-]*\z/ !~ encoding
        raise Hpricot::Error, "invalid encoding in XML declaration: #{encoding.inspect}"
      end
      unless standalone == nil || standalone == true || standalone == false
        raise Hpricot::Error, "invalid standalone document declaration in XML declaration: #{standalone.inspect}"
      end
      @version = version
      @encoding = encoding
  ...
  class DocType
    def initialize(root_element_name, public_identifier=nil, system_identifier=nil)
      ...
      if public_identifier && /\A[ \x0d\x0aa-zA-Z0-9\-'()+,.\/:=?;!*\#@$_%]*\z/ !~ public_identifier
        raise Hpricot::Error, "invalid public identifier in document type declaration: #{public_identifier.inspect}"
      end
      if system_identifier && /"/ =~ system_identifier && /'/ =~ system_identifier
        raise Hpricot::Error, "invalid system identifier in document type declaration: #{system_identifier.inspect}"
      end
  ...
  class ProcIns
    ...
    def initialize(target, content) # :notnew:
      init_raw_string
      if content && /\?>/ =~ content
        raise Hpricot::Error, "invalid processing instruction content: #{content.inspect}"
      end
      @target = target
      @content = content
    end
  ...
  class Comment
    ...
    def initialize(content) # :notnew:
      ...
      if /--/ =~ content || /-\z/ =~ content
        raise Hpricot::Error, "invalid comment content: #{content.inspect}"
      end
      @content = content
    end
    attr_reader :content
